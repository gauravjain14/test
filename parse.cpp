...
int* parse_graph_file(int *num_nodes, int *num_edges, char* tmpchar)
{
    ...
    while (fgets(line, 100, fptr)) {
        ...
        case 'p':
            ...
            size = (long long unsigned)(*num_nodes) * (long long unsigned)(*num_nodes);
            if (size > UINT_MAX) {
                fclose(fptr);
        ...
        case 'a':
            ...
            if (test_value(adjmatrix, *num_nodes, head-1, tail-1)) {
                set_value(adjmatrix, *num_nodes, head-1, tail-1, weight);
                cnt++;
            }

#ifdef VERBOSE
            printf("Adding edge: %d ==> %d ( %d )\n", head-1, tail-1, weight);
#endif
            break;
        default:
